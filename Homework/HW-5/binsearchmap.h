...
ArraySeq<K> BinSearchMap<K, V>::find_keys(const K &k1, const K &k2) const
{
    ...
    {
        int index = 0;
        if (bin_search(i, index))
        {
            new_seq.insert(seq[index].first, new_seq.size());
        }
    }
...
bool BinSearchMap<K, V>::bin_search(const K &key, int &index) const
{
    ...
    int low = 0;
    int high = new_seq.size() - 1;
    while (low <= high)
    {
        index = (low + high) / 2; // mid val
        ...
        if (new_seq[index] > key) // key is bigger than the middle, so it must be on the left side
        {
            high = index - 1;
        }
    }
    index = std::max(low, index);
    return false;
}