...
// FILE: arrayseq.h
// DATE: Fall 2021
// DESC: This file defines all of the functions defined for HW-3
//----------------------------------------------------------------------
...
ArraySeq<T> &ArraySeq<T>::operator=(ArraySeq<T> &&rhs)
{
    if (this != &rhs)
    {
        ...
        rhs.make_empty();
        rhs.array = nullptr; // to prevent memory leaks
    }
...
void ArraySeq<T>::resize()
{
    ...
    {
        capacity *= 2; // this is what makes resize so cool
    }
...
// This is a function i had to add
...
std::ostream &operator<<(std::ostream &stream, const ArraySeq<T> &array)
{
    for (int i = 0; i < array.size(); i++)
    {
        if (i != array.size() - 1)
        {
            stream << array[i] << ", "; // output the content of array at index i to the stream
        }
        else
        {
            stream << array[i]; // this is a case so the final index doesnt have a comma at the end
        }
    }
    return stream;
}
